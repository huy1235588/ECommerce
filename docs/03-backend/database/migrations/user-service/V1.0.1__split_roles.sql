...
CREATE OR REPLACE FUNCTION assign_role_to_user(p_user_id UUID, p_role_name VARCHAR)
RETURNS VOID AS $$
DECLARE
    v_role_id UUID;
BEGIN
...
CREATE OR REPLACE FUNCTION revoke_role_from_user(p_user_id UUID, p_role_name VARCHAR)
RETURNS VOID AS $$
DECLARE
    v_role_id UUID;
BEGIN
...
VALUES
    (gen_random_uuid(), 'CUSTOMER', 'Default customer role', true),
...
COMMENT ON TABLE roles IS 'System roles for RBAC; might be extended with permissions table later';
COMMENT ON COLUMN roles.name IS 'Canonical role name (e.g., customer, admin)';
COMMENT ON COLUMN roles.is_default IS 'Whether role should be assigned to new users by default';

COMMENT ON TABLE user_roles IS 'Mapping between users and roles; revoked_at indicates soft revocation';
COMMENT ON COLUMN user_roles.assigned_at IS 'Timestamp when role was assigned to user';
COMMENT ON COLUMN user_roles.revoked_at IS 'Timestamp when role was revoked (soft remove)';
