...
ALTER TABLE user_profiles
    ADD CONSTRAINT chk_user_profiles_country_format
...
CREATE
OR REPLACE FUNCTION update_user_profiles_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at
= NOW();
...
CREATE
OR REPLACE FUNCTION update_user_sessions_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE
OR REPLACE FUNCTION is_token_valid(
...
BEGIN
    IF
p_token_type = 'refresh' THEN
SELECT expires_at, is_revoked
INTO v_expires_at, v_is_revoked
FROM refresh_tokens
...
ELSIF
p_token_type = 'password_reset' THEN
SELECT expires_at, is_used
INTO v_expires_at, v_is_used
...
ELSIF
p_token_type = 'email_verification' THEN
SELECT expires_at, is_verified
INTO v_expires_at, v_is_used
...
COMMENT
ON TABLE users IS 'Authentication and account information for users';
COMMENT
ON COLUMN users.id IS 'Unique identifier (UUID)';
COMMENT
ON COLUMN users.email IS 'User email address (lowercased before insert)';
COMMENT
ON COLUMN users.username IS 'Unique username';
COMMENT
ON COLUMN users.password IS 'BCrypt hashed password';
COMMENT
ON COLUMN users.email_verified IS 'Email verification flag';

COMMENT
ON TABLE user_profiles IS 'Profile information for users';
COMMENT
ON COLUMN user_profiles.user_id IS 'Reference to users.id';
